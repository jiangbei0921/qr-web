...
> 应用已部署到 Render（免费层，新加坡区域），以下为生产环境公网地址，开箱即用：

- **生产环境（公网）**：https://smartcode-qr.onrender.com
- **本地预览**：http://localhost:5000

提示：Render 免费层在空闲约 15 分钟后会进入休眠，首次访问会自动唤醒（约 30 秒冷启动）。`main` 分支的任何推送都会通过 Auto-Deploy 自动重新部署。
